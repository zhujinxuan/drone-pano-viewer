...
## Amendment (2026-09-16, tickets 09/13/14/15/16/17 — render performance round)

- **The payload is served cached and gzipped (09/13).** The endpoint body (`{ layers: [...] }`, now with per-layer `vertices`) is serialized and `gzipSync`'d once per accepted state change, never per request; gzip is served when the client accepts it (measured 3.67 MB → 1.24 MB on the reporter set). A layer serving over 200 000 vertices logs a producer-side simplification hint per accepted load.
- **Detail is banded by camera distance (14).** Features with no vertex within 200 m render from a cam-independent 3 m ground-DP coarse geometry, precomputed once per payload and cached per feature — the tippecanoe/game-LOD model (simplify once per band, select per distance) instead of re-simplifying per camera. The served geometry stays verbatim; LOD is render-only. Band verdicts reuse the cull's underestimating planar scales, so borderline features always render full-fidelity.
- **Draw calls batch per layer; the build is progressive (15/16).** All strokes of a layer merge into one LineSegments, all fills into one Mesh (~2 draw calls/layer, measured 8 total vs ~10³ before). Layers build one per time slice, cheapest first, with a generation counter cancelling stale slices; the toolbar pulses a building layer's name. Hidden layers never build.
- **Heavy layers seed hidden (17).** Over 50 000 served vertices → unchecked by default with an explanatory tooltip; user toggles survive refetches. Detection reuses the payload's `vertices`, not a second metric.
- **Prop identity is an effect contract (15).** The smoke exposed App's unmemoized `camFix`/`groundCam` restarting the overlay build on every 10 Hz HUD tick — the dominant drag-time cost all along. Memoized on value identity; recorded here because the same class of bug (fresh object → effect restart) applies to every future overlay prop.
